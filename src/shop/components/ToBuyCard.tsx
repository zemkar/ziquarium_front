...
const SaleCard = () => {
  ...
  const toBuy = () => {
    console.log("toBuy:", isSaleDataShow?.itemId, amount, sellData?.price, itemData?.name);

  };

  const toCart = () => {
    if (amount >= 0) {
      if (sellData?.quantity_for_discount <= amount) {
        dispatch(cartItemAmount(isSaleDataShow?.itemId, amount, sellData?.price, sellData?.quantity_discount))
      }
      else {
        dispatch(cartItemAmount(isSaleDataShow?.itemId, amount, sellData?.price))
      }
  ...
  return (
    <div>
      {sellData?.price > 0 ? <>
        <input type='number' value={amount || 0} onChange={(e) => { setAmount(+e.target.value) }} min={0} max={sellData?.quantity}></input> {totalToPay} 
        ...
        <hr />
        Price: {sellData?.price}  &nbsp; &nbsp;  | &nbsp; In stock: {sellData?.quantity}
        <hr />
